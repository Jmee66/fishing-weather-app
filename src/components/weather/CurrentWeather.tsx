  ...
  return (
    <Card className="space-y-4">
      <div className="flex items-start justify-between">
        <div>
          <div className="text-5xl font-light text-slate-100">{formatTemperature(c.temp, units)}</div>
          <div className="text-slate-500 text-sm mt-1">Ressenti {formatTemperature(c.feels_like, units)}</div>
          <div className="text-slate-200 font-medium mt-1 capitalize">{c.weather[0]?.description ?? ''}</div>
        </div>
      ...
      <div className="flex justify-between text-xs text-slate-500 pt-1 border-t border-[var(--border-subtle)]">
