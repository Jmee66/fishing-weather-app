...
export default function WindWidget({ windSpeed, windDeg, windGust, compact = false }: WindWidgetProps) {
  ...
  return (
    <div className={`flex items-center gap-3 ${compact ? '' : 'p-3 bg-slate-50 rounded-xl'}`}>
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-baseline gap-1">
          <span className="text-xl font-bold text-slate-800">{formatWindSpeed(windSpeed, units)}</span>
          {windGust && (
